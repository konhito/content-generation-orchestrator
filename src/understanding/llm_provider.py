...
class OpenAILLMProvider(LLMProvider):
    ...
    @property
    def client(self) -> Any:
        """Create the SDK client lazily so configuration can be inspected offline."""
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "The openai and python-dotenv packages are required. "
    ...
    def generate_json(
        ...
        output = self.generate(json_prompt, system_prompt)
        try:
            result = json.loads(output)
        except json.JSONDecodeError as exc:
            raise ValueError(f"OpenAI returned invalid JSON: {exc}") from exc
        if not isinstance(result, dict):
            raise ValueError("OpenAI returned invalid JSON: expected an object")
        return result
