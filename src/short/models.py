...
class ShortBeatMode(str, Enum):
    """Full-frame visual modes supported by the Shorts player."""
...
class CharacterEvent(BaseModel):
    """A continuous character performance interval."""
...
class MouthCue(BaseModel):
    """A mouth-shape interval relative to a character beat."""
...
class BlinkCue(BaseModel):
    """A blink interval relative to a character beat."""
...
class CharacterTrack(BaseModel):
    """Validated character performance data for one Shorts beat."""

    version: int = 1
    character_id: str
    duration_seconds: float
    base_pose: str = "body1"
    base_emotion: str = "content"
    events: list[CharacterEvent] = Field(default_factory=list)
    mouth_cues: list[MouthCue] = Field(default_factory=list)
    blink_cues: list[BlinkCue] = Field(default_factory=list)

    @model_validator(mode="after")
    def validate_event_intervals(self) -> "CharacterTrack":
        previous_end = 0.0
        for event in sorted(self.events, key=lambda item: item.start):
            if event.start < 0 or event.end <= event.start:
                raise ValueError("character event has an invalid interval")
            if event.end > self.duration_seconds:
                raise ValueError("character event exceeds track duration")
            if event.start < previous_end:
                raise ValueError("character events overlap")
...
class VisualType(str, Enum):
    ...
    MEME_CARD = "meme_card"  # Structured meme card with top/bottom captions
