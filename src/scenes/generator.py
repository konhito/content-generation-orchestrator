...
class SceneGenerator:
    ...
    def generate_all_scenes(
        ...
        if voiceover_manifest_path and voiceover_manifest_path.exists():
            with open(voiceover_manifest_path, encoding="utf-8") as f:
    ...
    def _sync_scene_timing(
        ...
        # Use the configured LLM to sync timing
        llm = get_llm_provider(load_config())

        full_prompt = f"""{prompt}

Return only the complete updated TSX component code in a code block.
"""
        response = llm.generate(full_prompt, SCENE_SYSTEM_PROMPT)
...
{validation_feedback}
Return only the complete TSX component code in a code block. Do not describe it.
"""
        response = llm.generate(full_prompt, SCENE_SYSTEM_PROMPT)
        code = self._extract_code(response)
        if not code:
            raise RuntimeError(f"Scene code was not returned for: {output_path}")
    ...
    def _load_example_scene(self, example_dir: Path | None = None) -> str:
        ...
        for dir_path in example_dirs:
            ...
            if tsx_files:
                with open(tsx_files[0], encoding="utf-8") as f:
                    return f.read()
