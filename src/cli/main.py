...
def cmd_scenes(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        syntax_verification = results.get("syntax_verification")
        if syntax_verification and not syntax_verification.get("success", False):
            print("Error: Generated scenes contain unresolved syntax errors.", file=sys.stderr)
            print("Regenerate the scenes before rendering.", file=sys.stderr)
            return 1
...
def cmd_short(args: argparse.Namespace) -> int:
    ...
    short_first = _is_short_first_args(args)
    ...
    try:
        ...
        if not short_first:
            print(f"Error: {e}", file=sys.stderr)
            return 1
        title = getattr(args, "topic", None) or args.project.replace("-", " ").title()
        project = create_project(args.project, title=title, projects_dir=args.projects_dir)

    source_script = None
    if not short_first:
        # Validate prerequisites for the existing long-project-to-short flow.
        narration_path = project.get_path("narration")
        if not narration_path.exists():
    ...
    if short_first:
        from ..short.first import ShortFirstGenerator
        ...
        result = first_generator.generate(
            ...
            niche=getattr(args, "niche", "default"),
...
def _is_short_first_args(args: argparse.Namespace) -> bool:
    """Return True only when real short-first flags are present.

    Some tests use MagicMock namespaces where missing attributes evaluate as
    truthy MagicMock objects. Keep this check strict so legacy short commands do
    not accidentally take the topic/source path.
    """

    topic = getattr(args, "topic", None)
    source = getattr(args, "source", None)
    research = getattr(args, "research", False)
    has_topic = isinstance(topic, str) and bool(topic.strip())
    has_source = isinstance(source, list) and bool(source)
    has_research = isinstance(research, bool) and research
    return has_topic or has_source or has_research
...
def _apply_component_plan_to_storyboard(shorts_storyboard, component_plan_path: Path):
    ...
    plan = json.loads(component_plan_path.read_text(encoding="utf-8"))
    character_config = load_config().character
    ...
    for item in components:
        ...
        scene_config_data = visual_data.get("scene_config") or {}
        scene_config = None
        if scene_config_data:
            scene_config = SceneComponentConfig(
                component_type=str(scene_config_data.get("component_type", visual_type_str)),
                props=scene_config_data.get("props", {}),
                tokens=scene_config_data.get("tokens", []),
                mode=scene_config_data.get("mode", "prefill"),
                rows=scene_config_data.get("rows", 4),
                cols=scene_config_data.get("cols", 4),
                bars=scene_config_data.get("bars", []),
                code=scene_config_data.get("code", ""),
                language=scene_config_data.get("language", "python"),
                highlight_lines=scene_config_data.get("highlight_lines", []),
                image_path=scene_config_data.get("image_path", ""),
                caption=scene_config_data.get("caption", ""),
                highlight_indices=scene_config_data.get("highlight_indices", []),
                dimensions=scene_config_data.get("dimensions", 8),
                values=scene_config_data.get("values", []),
                size=scene_config_data.get("size", 6),
    ...
    if character_config.enabled and any(beat.mode == ShortBeatMode.CHARACTER for beat in new_beats):
        ...
        if canonical_assets.exists():
            shutil.copytree(canonical_assets, installed_assets, dirs_exist_ok=True)
            ...
            warnings = attach_character_tracks(
                ...
                audio_path=(project_root / shorts_storyboard.voiceover_path) if shorts_storyboard.voiceover_path else None,
                aligner_url=character_config.aligner_url,
                aligner_timeout=character_config.aligner_timeout_seconds,
            )
            validation_path = variant_dir / "character/validation.json"
...
def cmd_short_script(args: argparse.Namespace) -> int:
    ...
    if short_first:
        ...
        duration = args.duration or 50
        topic = getattr(args, "topic", None) or project.title
        source_paths = [Path(path) for path in (getattr(args, "source", None) or [])]
        result = ShortFirstGenerator().generate(
            project_id=project.id,
            project_dir=project.root_dir,
            topic=topic,
            source_paths=source_paths,
        ...
        print(f"Generated short-first script: {result.short_script_path}")
        print(f"Meme plan: {result.meme_plan_path}")
        return 0
...
def cmd_short_research(args: argparse.Namespace) -> int:
    """Create the Shorts research bundle only."""
    import json
    from ..project.loader import create_project, load_project
    from ..short.first import ShortFirstGenerator, _bundle_to_dict

    project_path = Path(args.projects_dir) / args.project
    try:
        project = load_project(project_path)
    except FileNotFoundError:
        title = args.topic or args.project.replace("-", " ").title()
        project = create_project(args.project, title=title, projects_dir=args.projects_dir)

    generator = ShortFirstGenerator()
    topic = args.topic or project.title
    ...
    print(f"Generated shorts research bundle: {output_path}")
    return 0


def cmd_short_beats(args: argparse.Namespace) -> int:
    ...
    if not short_script_path.exists():
        print("Error: Short script not found. Run 'short script' first.", file=sys.stderr)
        return 1
    short_script = ShortGenerator.load_short_script(short_script_path)
    beats = build_script_beats(short_script.condensed_narration, args.niche)
    output_path = variant_dir / "beats" / "script_beats.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps({"beats": beats}, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"Generated shorts script beats: {output_path}")
    print(f"Beats: {len(beats)}")
    return 0


def cmd_short_memes(args: argparse.Namespace) -> int:
    ...
    variant_dir = project.short_dir / args.variant
    short_script_path = variant_dir / "short_script.json"
    beats_path = variant_dir / "beats" / "script_beats.json"
    if not short_script_path.exists() or not beats_path.exists():
        print("Error: Missing short_script or script_beats. Run short script/beats first.", file=sys.stderr)
        return 1
    short_script = ShortGenerator.load_short_script(short_script_path)
    beats = json.loads(beats_path.read_text(encoding="utf-8")).get("beats", [])
    editing = get_editing_config(load_niche(args.niche))
    low, high = editing.get("meme_beats", [1, 3])
    target = max(low, min(high, 3))
    raw_memes = [
        {
            "type": "meme",
            "query": beat.get("script_text", "reaction"),
            "template_hint": "surprised" if index % 2 == 0 else "choice",
...
def cmd_short_components(args: argparse.Namespace) -> int:
    ...
    short_script_path = variant_dir / "short_script.json"
    beats_path = variant_dir / "beats" / "script_beats.json"
    meme_path = variant_dir / "memes" / "meme_plan.json"
    if not short_script_path.exists() or not beats_path.exists() or not meme_path.exists():
        print("Error: Missing short_script, script_beats, or meme_plan. Run short script/beats/memes first.", file=sys.stderr)
        return 1
    short_script = ShortGenerator.load_short_script(short_script_path)
    beats = json.loads(beats_path.read_text(encoding="utf-8")).get("beats", [])
    memes = json.loads(meme_path.read_text(encoding="utf-8")).get("moments", [])
    plan = build_component_plan(beats, memes, duration=short_script.total_duration_seconds, niche=args.niche)
    output_path = variant_dir / "components" / "component_plan.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(plan, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"Generated shorts component plan: {output_path}")
...
Use --interactive to pause for plan review before continuing.
    ...
    generate_parser.add_argument(
        ...
        default="edge",
        help="TTS provider for voiceover step (default: edge)",
    )
    generate_parser.add_argument(
        "--mock",
        action="store_true",
...
Or run individual steps:
  python -m src.cli short research <project>    # 1. Research/source bundle
  python -m src.cli short script <project>      # 2. Niche-shaped script
  python -m src.cli short beats <project>       # 3. Attention-span beats
  python -m src.cli short memes <project>       # 4. Structured meme plan
  python -m src.cli short components <project>  # 5. Clean Remotion component plan
  python -m src.cli short scenes <project>      # 6. Scene setup
  python -m src.cli short voiceover <project>   # 7. Voiceover
  python -m src.cli short storyboard <project>  # 8. Create storyboard
  python -m src.cli short timing <project>      # 9. Generate timing.ts (optional)
...
For manual voiceover recording:
  ...
  python -m src.cli short voiceover <project> --audio <recording.mp3>
    ...
    short_generate_parser.add_argument(
        "--topic",
        help="Short-first topic. When set, does not require an existing long-video script.",
    )
    short_generate_parser.add_argument(
        "--source",
        action="append",
        help="Short-first source file (Markdown/text). Can be passed multiple times.",
    )
    short_generate_parser.add_argument(
        "--niche",
        default="default",
        help="Short-first niche profile (default: default, e.g. tech, comedy, finance)",
    )
    short_generate_parser.add_argument(
        "--research",
        action="store_true",
    ...
    # short research
    short_research_parser = short_subparsers.add_parser(
        "research",
        help="Generate short research/source bundle",
    )
    short_research_parser.add_argument("project", help="Project ID")
    short_research_parser.add_argument("--variant", default="default", help="Variant name")
    short_research_parser.add_argument("--topic", help="Short topic")
    short_research_parser.add_argument("--source", action="append", help="Source file. Can be passed multiple times.")
    short_research_parser.add_argument("--niche", default="general", help="Niche profile")
    short_research_parser.add_argument("--research", action="store_true", help="Include research seed notes")
    short_research_parser.set_defaults(func=cmd_short_research)

    # short beats
    short_beats_parser = short_subparsers.add_parser(
        "beats",
        help="Generate attention-span script beats",
    )
    short_beats_parser.add_argument("project", help="Project ID")
    short_beats_parser.add_argument("--variant", default="default", help="Variant name")
    short_beats_parser.add_argument("--niche", default="general", help="Niche profile")
    short_beats_parser.set_defaults(func=cmd_short_beats)

    # short memes
    short_memes_parser = short_subparsers.add_parser(
        "memes",
        help="Generate structured meme plan",
    )
    short_memes_parser.add_argument("project", help="Project ID")
    ...
    short_components_parser = short_subparsers.add_parser(
        "components",
        help="Generate clean Remotion component plan",
    )
    short_components_parser.add_argument("project", help="Project ID")
    short_components_parser.add_argument("--variant", default="default", help="Variant name")
    short_components_parser.add_argument("--niche", default="general", help="Niche profile")
    short_components_parser.set_defaults(func=cmd_short_components)
