...
async function main() {
  ...
  if (config.storyboardPath) {
    ...
    if (!projectDir) {
      projectDir = resolve(deriveProjectDir(config.storyboardPath));
    }
  ...
  const projectScenesAlias = skipSceneValidation && !existsSync(projectScenesDir)
    ? resolve(__dirname, "../src/scenes/emptyProjectScenes.ts")
    : projectScenesDir;
  ...
  if (projectScenesAlias !== projectScenesDir) {
    console.log(`Using empty scene registry for ${config.compositionId}`);
  }
